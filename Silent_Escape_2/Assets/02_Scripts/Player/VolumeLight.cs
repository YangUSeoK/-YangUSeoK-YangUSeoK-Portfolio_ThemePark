...
public class VolumeLight : MonoBehaviour
{
    ...
    {
        float n = 1f / (m_SubDivision - 1f);
        float radius = Mathf.Tan((m_Light.spotAngle * 0.5f) * Mathf.Deg2Rad); // 반지름
        float length = m_Light.range / Mathf.Cos(m_Light.spotAngle * 0.5f * Mathf.Deg2Rad); // 레이 쏠 길이. 원뿔의 모선
        Vector3 origCircleVert;     // z축을 중심으로하는 원의 vertext
        Vector3 newCircleVert;      // 회전행렬을 통해 회전한 원의 vertext

        List<Vector3> lightVertList = new List<Vector3>();  // 실제로 메쉬를 만들 vertex List
        ...
        for (int i = 0; i < m_SubDivision; ++i)
        {
            ...
            // 각각의 점 위치를 향해 length 길이만큼 레이를 쏜다. 
    ...
    private void BuildMesh(List<Vector3> _lightVertList)
    {
        ...
        // 20221106 양우석 : 메쉬 만드는 함수는 따로 쪼갤 것 >> 20221109 쪼갰음
        // +1 해주는 이유 : 맨 처음 시작점(자신의 위치) 개수를 추가해줘야 한다.
